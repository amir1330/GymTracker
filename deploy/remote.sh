#!/bin/sh
set -eu
APP_DIR="${GYMTRACKER_APP:-/home/gymtracker/gym-tracker}"
cd "$APP_DIR"
...
# Keep the historical compose project so gym-tracker_postgres_data stays attached.
export COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-gym-tracker}"

echo "Pulling images..."
$DC $FILE pull gym-tracker
docker logout ghcr.io >/dev/null 2>&1 || true

echo "Recreating gym-tracker (db volume untouched)..."
docker stop gym-tracker 2>/dev/null || true
docker rm gym-tracker 2>/dev/null || true
$DC $FILE up -d --no-build --no-deps --force-recreate gym-tracker
