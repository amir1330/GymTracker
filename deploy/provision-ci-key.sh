...
# - Creates gymtracker on the VPS (docker group, app dir)
# - Installs forced-command authorized_keys entry (sync-deploy / deploy only)
# - Sets GitHub secrets HOST / USERNAME / SSH_KEY
# Idempotent: replaces previous github-actions-gymtracker keys.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
REPO="${GITHUB_REPO:-amir1330/GymTracker}"
VPS="${DEPLOY_HOST:-root@89.167.79.221}"
HOST_ONLY="${DEPLOY_IP:-89.167.79.221}"
USER_NAME=gymtracker
KEY_COMMENT="github-actions-gymtracker"
